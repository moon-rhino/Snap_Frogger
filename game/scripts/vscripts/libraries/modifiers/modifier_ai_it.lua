...
function modifier_ai_it:OnCreated(params)
    ...
    if IsServer() then
        ...
        -- unit:AddNewModifier(caster, ability, "modifier_ai_it", { aggroRange = X, leashRange = Y })
...
function modifier_ai_it:AggressiveThink()
    ...
    if not self.aggroTarget:IsAlive() then
        ...
        --self.unit:MoveToPosition(self.unit.spawnVector) --Move back to the spawnpoint
        self.state = AI_STATE_IDLE --Transition the state to the 'Returning' state(!)
        return -- Stop processing this state
    end
...
--[[function modifier_ai_it:ReturningThink()
