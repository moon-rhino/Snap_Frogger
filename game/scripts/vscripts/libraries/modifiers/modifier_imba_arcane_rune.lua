LinkLuaModifier("modifier_imba_arcane_rune_aura", "libraries/modifiers/modifier_imba_arcane_rune.lua", LUA_MODIFIER_MOTION_NONE)
--LinkLuaModifier("modifier_imba_arcane_rune_super", "libraries/modifiers/modifier_imba_arcane_rune.lua", LUA_MODIFIER_MOTION_NONE)
...
--aura is removed with the modifier
function modifier_imba_arcane_rune:OnDeath( params )
	print("[modifier_imba_arcane_rune:OnDeath] inside the function")
	
	if IsServer() then
		if params.unit == self:GetParent() then
			self:GetParent():RemoveModifierByName("modifier_imba_arcane_rune")
		end
	end
	return 0
end
...
--[[
--if not done, effect will linger even after death
function modifier_imba_arcane_rune_aura:OnDeath( params )
    print("[modifier_imba_arcane_rune_aura:OnDeath] inside the function")
	if IsServer() then
		if params.unit == self:GetParent() then
			self:GetParent():RemoveModifier("modifier_imba_arcane_rune_aura")
		end
	end
	return 0
end]]