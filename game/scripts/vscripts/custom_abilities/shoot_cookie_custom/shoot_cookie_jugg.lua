...
function shoot_cookie_jugg:OnSpellStart()
    if IsServer() then
        ...
        { 
            Ability = self,
            EffectName = "particles/hero_snapfire_cookie_projectile_linear_jugg_perp_down.vpcf", --particle effect
...
function shoot_cookie_jugg:OnProjectileHit(hTarget, vLocation)
    if hTarget == nil then
        return false
        --skip
    ...
    elseif hTarget:GetTeamNumber() ~= self.caster:GetTeamNumber() then
        ...
        for cookiePadIndex, cookiePad in pairs(cookiePads) do
            if string.sub(cookiePad:GetUnitName(), 12) == "jugg" then
                return false
            end
        end
...
function shoot_cookie_jugg:PlayEffects3( target, radius )
	-- Get Resources
