...
function shoot_cookie_ogre:OnSpellStart()
    if IsServer() then
        ...
        { 
            Ability = self,
            EffectName = "particles/hero_snapfire_cookie_projectile_linear_ogre_perp_down.vpcf", --particle effect
...
function shoot_cookie_ogre:OnProjectileHit(hTarget, vLocation)
    if hTarget == nil then
        return false
        --skip
    ...
    elseif hTarget:GetTeamNumber() ~= self.caster:GetTeamNumber() then
        ...
        for cookiePadIndex, cookiePad in pairs(cookiePads) do
            if string.sub(cookiePad:GetUnitName(), 12) == "ogre" then
                return false
            end
        end
...
function shoot_cookie_ogre:PlayEffects3( target, radius )
	-- Get Resources
