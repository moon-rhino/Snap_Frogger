...
mortimer_kisses_morty = class({})

LinkLuaModifier( "mortimer_kisses_morty_modifier", "custom_abilities/mortimer_kisses_morty", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "mortimer_kisses_morty_thinker_modifier", "custom_abilities/mortimer_kisses_morty", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "magma_burn_slow_modifier", "custom_abilities/mortimer_kisses_morty", LUA_MODIFIER_MOTION_NONE )


--------------------------------------------------------------------------------
-- Custom KV
-- AOE Radius
function mortimer_kisses_morty:GetAOERadius()
	return self:GetSpecialValueFor( "impact_radius" )
end

--------------------------------------------------------------------------------
-- Ability Start
function mortimer_kisses_morty:OnSpellStart()
	--sound
	local randomNumber = math.random(1, 2)
	if randomNumber == 1 then
		EmitSoundOn("burp", self:GetCaster())
	else
		EmitSoundOn("burp_2", self:GetCaster())
	end
	...
	caster:AddNewModifier(
		...
		self, -- ability source
		"mortimer_kisses_morty_modifier", -- modifier name
		{
			duration = duration,
			pos_x = point.x,
			pos_y = point.y,
...
--------------------------------------------------------------------------------
-- Projectile
function mortimer_kisses_morty:OnProjectileHit( target, location )
	...
	local mod = target:AddNewModifier(
		...
		self, -- ability source
		"mortimer_kisses_morty_thinker_modifier", -- modifier name
		{
			duration = duration,
			slow = 1,
		} -- kv
	...
	-- epicenter
	-- channeled ability
	-- ability phase start, on spell start, on channel finish
	-- the main ability starts on channel finish
	local epicenterUnit = CreateUnitByName("dummy", location, true, self:GetCaster(), nil, self:GetCaster():GetTeam())
...
--------------------------------------------------------------------------------
function mortimer_kisses_morty:PlayEffects( loc )
	-- Get Resources
...
--------------------------------------------------------------------------------
mortimer_kisses_morty_modifier = class({})

--------------------------------------------------------------------------------
-- Classifications
function mortimer_kisses_morty_modifier:IsHidden()
	return false
end

function mortimer_kisses_morty_modifier:IsDebuff()
	return false
end

function mortimer_kisses_morty_modifier:IsStunDebuff()
	return false
end

function mortimer_kisses_morty_modifier:IsPurgable()
	return false
end

--------------------------------------------------------------------------------
-- Initializations
function mortimer_kisses_morty_modifier:OnCreated( kv )
	-- references
...
function mortimer_kisses_morty_modifier:OnRefresh( kv )
	
end

function mortimer_kisses_morty_modifier:OnRemoved()
end

function mortimer_kisses_morty_modifier:OnDestroy()
end

--------------------------------------------------------------------------------
-- Modifier Effects
function mortimer_kisses_morty_modifier:DeclareFunctions()
	local funcs = {
...
function mortimer_kisses_morty_modifier:OnOrder( params )
...
function mortimer_kisses_morty_modifier:GetModifierMoveSpeed_Limit()
	return 0.1
end

function mortimer_kisses_morty_modifier:GetModifierTurnRate_Percentage()
	return -self.turn_rate
end

--------------------------------------------------------------------------------
-- Status Effects
function mortimer_kisses_morty_modifier:CheckState()
	local state = {
...
--------------------------------------------------------------------------------
-- Interval Effects
function mortimer_kisses_morty_modifier:OnIntervalThink()
	self:CreateBlob(kv)
end

function mortimer_kisses_morty_modifier:CreateBlob(kv)
...
--------------------------------------------------------------------------------
-- Helper
function mortimer_kisses_morty_modifier:SetValidTarget( location )
...
--------------------------------------------------------------------------------
mortimer_kisses_morty_thinker_modifier = class({})
...
--------------------------------------------------------------------------------
-- Initializations
function mortimer_kisses_morty_thinker_modifier:OnCreated( kv )
    print("[mortimer_kisses_morty_thinker_modifier:OnCreated( kv )] called")
    print("[mortimer_kisses_morty_thinker_modifier:OnCreated( kv )] ability that called this function: " .. self:GetAbility():GetAbilityName())
	-- references
	self.max_travel = self:GetAbility():GetSpecialValueFor( "max_lob_travel_time" )
    ...
    if self:GetAbility():GetAbilityName() == "mortimer_kisses_morty" then
        self:PlayEffects( kv.travel_time )
    end
end

function mortimer_kisses_morty_thinker_modifier:OnRefresh( kv )
	-- references
	self.max_travel = self:GetAbility():GetSpecialValueFor( "max_lob_travel_time" )
	self.radius = self:GetAbility():GetSpecialValueFor( "impact_radius" )
	self.linger = self:GetAbility():GetSpecialValueFor( "burn_linger_duration" )
...
function mortimer_kisses_morty_thinker_modifier:OnRemoved()
end

function mortimer_kisses_morty_thinker_modifier:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Aura Effects
function mortimer_kisses_morty_thinker_modifier:IsAura()
	return self.start
end

function mortimer_kisses_morty_thinker_modifier:GetModifierAura()
	return "magma_burn_slow_modifier"
end

function mortimer_kisses_morty_thinker_modifier:GetAuraRadius()
	return self.radius
end

function mortimer_kisses_morty_thinker_modifier:GetAuraDuration()
	return self.linger
end

function mortimer_kisses_morty_thinker_modifier:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function mortimer_kisses_morty_thinker_modifier:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function mortimer_kisses_morty_thinker_modifier:PlayEffects( time )
	-- Get Resources
...
function mortimer_kisses_morty_thinker_modifier:StopEffects()
