...
function shoot_cookie_axe:OnSpellStart()
    if IsServer() then
        ...
        { 
            Ability = self,
            EffectName = "particles/hero_snapfire_cookie_projectile_linear_axe_perp_down.vpcf", --particle effect
...
function shoot_cookie_axe:OnProjectileHit(hTarget, vLocation)
    if hTarget == nil then
        return false
        --skip
...
function shoot_cookie_axe:PlayEffects3( target, radius )
	-- Get Resources
