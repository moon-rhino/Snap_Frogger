...
function shoot_cookie_bb:OnSpellStart()
    if IsServer() then
        ...
        { 
            Ability = self,
            EffectName = "particles/hero_snapfire_cookie_projectile_linear_bb_perp_down.vpcf", --particle effect
...
function shoot_cookie_bb:OnProjectileHit(hTarget, vLocation)
    if hTarget == nil then
        return false
        --skip
    ...
    elseif hTarget:GetTeamNumber() ~= self.caster:GetTeamNumber() then
        ...
        for cookiePadIndex, cookiePad in pairs(cookiePads) do
            if string.sub(cookiePad:GetUnitName(), 12) == "bb" then
                return false
            end
        end
...
function shoot_cookie_bb:PlayEffects3( target, radius )
	-- Get Resources
